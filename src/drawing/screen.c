...
void get_texture(t_cubed *cubed, int x, int y, double wall_height, double wall_width)
{
	double texture_x;
	double texture_y;
	// texture_x = (double)((x - wall_position)) / wall_width;
	texture_x = (double)(x) / 64;
	...
	mlx_put_pixel(cubed->image_game, x, y, texture_color);
}

double	get_wall_width(t_cubed *cubed, size_t *wall_position)
{
	size_t wall_width = 1;
	size_t wall_end_pos = (*wall_position) + 1;
...
void    draw_wall(t_cubed *cubed, size_t *wall_position)
{
	...
	// uint32_t color = get_color(cubed);
	double wall_width = get_wall_width(cubed, wall_position);
	while (y < end_y)
	{
		x = *wall_position;
		...
		while (x < (*wall_position) + 1)
		{
			if (x < mini_map_surface && y < mini_map_surface)
				...
				get_texture(cubed, x, y, wall_height, wall_width);
